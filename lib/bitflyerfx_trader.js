...
class BitflyerfxTrader extends Trader {
	...
	*/
	 ...
	 * 約定履歴ストリーム.
	 */
	async executionsStream ( receiver ) {
		var self = this;
		this.__createStreamApi( 'lightning_executions_' + self.pair.toUpperCase(), function( data ){
			receiver( data );
		} );
	}
	 ...
	 */
	__createStreamApi ( channelName, receiver){
		// promiseにはしてるけどthen呼ばれないようにしてる（resolve実行しない. コネクションクローズで呼ぶとかしてもいいかも）
		return new Promise(( resolve, reject )=>{
			let ws = new WebSocket("wss://ws.lightstream.bitflyer.com/json-rpc");
			ws.on("open", () => {
				ws.call("subscribe", {
					channel: channelName
				});
